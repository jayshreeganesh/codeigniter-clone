    ...
    <div class="p-6 border-b border-slate-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div class="flex gap-2">
            <?php if(isset($_SESSION['user_id'])): ?>
                ...
                <a href="<?= site_url('logout') ?>" class="inline-flex items-center px-4 py-2 bg-slate-200 hover:bg-slate-300 text-slate-700 text-sm font-medium rounded-lg shadow-sm transition">Logout</a>
            <?php else: ?>
                    ...
                    <?php foreach ($products as $product): ?>
                        <tr class="hover:bg-slate-50/75 transition">
                            ...
                            <td class="py-4 px-6 text-right space-x-2 whitespace-nowrap">
                                <a href="<?= site_url('products/' . $product->id) ?>" class="inline-flex items-center p-1.5 text-slate-500 hover:text-red-600 hover:bg-red-50 rounded transition" title="Show">
                                    <i class="fa-regular fa-eye"></i>
                                </a>
                                <a href="<?= site_url('products/' . $product->id . '/edit') ?>" class="inline-flex items-center p-1.5 text-slate-500 hover:text-amber-600 hover:bg-amber-50 rounded transition" title="Edit">
                                    <i class="fa-regular fa-pen-to-square"></i>
                                </a>
                                <form action="<?= site_url('products/' . $product->id . '/delete') ?>" method="POST" class="inline-block" onsubmit="return confirm('Delete this product?');">
                                    <?= csrf_field() ?>
