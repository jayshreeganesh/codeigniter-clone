<div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
    <div class="p-6 border-b border-slate-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            ...
            <p class="text-sm text-slate-500 mt-1">CodeIgniter MVC</p>
        </div>
        <div class="flex gap-2">
            <?php if($this->session->userdata('user_id')): ?>
                <span class="px-4 py-2 text-sm text-slate-600 font-bold bg-slate-100 rounded-lg">Welcome, <?= htmlspecialchars($this->session->userdata('user_name')) ?></span>
                <a href="/auth/logout" class="inline-flex items-center px-4 py-2 bg-slate-200 hover:bg-slate-300 text-slate-700 text-sm font-medium rounded-lg shadow-sm transition">Logout</a>
            <?php else: ?>
    ...
    <div class="px-6 py-4 border-b border-slate-100 bg-slate-50">
        <form method="GET" action="/products" class="flex gap-2">
            ...
            <?php if(!empty($q)): ?><a href="<?= site_url('products') ?>" class="px-4 py-2 bg-slate-200 rounded-lg">Clear</a><?php endif; ?>
        </form>
    </div>
    <?php endif; ?>
    <!-- Action Buttons -->
    <div class="px-6 pb-4 bg-slate-50 border-b border-slate-100 flex flex-wrap gap-2 justify-between items-center">
        <div class="flex gap-2">
            <?php if (!empty($products)): ?>
                <a href="<?= site_url('products/export') ?>?format=csv&q=<?= urlencode($q ?? '') ?>" class="inline-flex items-center px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-medium rounded-lg shadow-sm transition">
                    <i class="fa-solid fa-file-csv mr-2"></i> Export CSV (Excel)
                </a>
                <a href="<?= site_url('products/export') ?>?format=json&q=<?= urlencode($q ?? '') ?>" class="inline-flex items-center px-4 py-2 bg-slate-800 hover:bg-slate-900 text-white text-sm font-medium rounded-lg shadow-sm transition">
        ...
        <a href="<?= site_url('products/create') ?>" class="inline-flex items-center px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium rounded-lg shadow-sm transition">
                ...
                <tbody class="divide-y divide-slate-100 text-sm">
                    <?php foreach ($products as $product): ?>
                        <tr class="hover:bg-slate-50/75 transition">
                            ...
                            <td class="py-4 px-6">
                                <a href="<?= site_url('products/' . $product->id) ?>" class="font-semibold text-slate-800 hover:text-red-600 transition">
                            ...
                            <td class="py-4 px-6 text-right space-x-2 whitespace-nowrap">
                                <?php if (!isset($trash) || !$trash): ?>
                                    <form action="<?= site_url('products/toggle_active/' . $product->id) ?>" method="POST" class="inline-block">
                                    ...
                                    <a href="<?= site_url('products/edit/' . $product->id) ?>" class="inline-flex items-center p-1.5 text-slate-500 hover:text-amber-600 hover:bg-amber-50 rounded transition" title="Edit">
                                        <i class="fa-regular fa-pen-to-square"></i>
                                    </a>
                                    <form action="<?= site_url('products/delete/' . $product->id) ?>" method="POST" class="inline-block" onsubmit="return confirm('Move this product to trash?');">
                                ...
                                <?php else: ?>
                                    <form action="<?= site_url('products/restore/' . $product->id) ?>" method="POST" class="inline-block">
                                    ...
                                    <form action="<?= site_url('products/force_delete/' . $product->id) ?>" method="POST" class="inline-block" onsubmit="return confirm('WARNING: Permanently delete this product? This cannot be undone!');">
